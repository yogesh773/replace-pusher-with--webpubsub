...
exports.WebPubSubClient = void 0;
...
const websocketClient_js_1 = require("./websocket/websocketClient.js");
const abortablePromise_js_1 = require("./utils/abortablePromise.js");
...
class WebPubSubClient {
    ...
    async _startFromRestarting(abortSignal) {
        ...
        try {
            logger_js_1.logger.verbose("Staring reconnecting.");
            await this._startCore(abortSignal);
        }
        catch (err) {
    ...
    async _startCore(abortSignal) {
        ...
        logger_js_1.logger.info("Staring a new connection");
    ...
    _getWebSocketClientFactory() {
        return new websocketClient_js_1.WebSocketClientFactory();
    }
    ...
    _connectCore(uri) {
        ...
        return new Promise((resolve, reject) => {
            ...
            client.onopen(() => {
                ...
                logger_js_1.logger.verbose("WebSocket connection has opened");
            ...
            client.onclose((code, reason) => {
                if (this._state === WebPubSubClientState.Connected) {
                    logger_js_1.logger.verbose("WebSocket closed after open");
                    if (this._sequenceAckTask != null) {
                        this._sequenceAckTask.abort();
                    }
                    logger_js_1.logger.info(`WebSocket connection closed. Code: ${code}, Reason: ${reason}`);
                    this._lastCloseEvent = { code: code, reason: reason };
                    this._handleConnectionClose.call(this);
                }
                else {
                    logger_js_1.logger.verbose("WebSocket closed before open");
            ...
            client.onmessage((data) => {
                const handleAckMessage = (message) => {
                    if (this._ackMap.has(message.ackId)) {
                        ...
                        if (message.success || isDuplicated) {
                            ...
                            entity.reject(new index_js_1.SendMessageError("Failed to send message.", {
                                ackId: message.ackId,
                ...
                try {
                    ...
                    logger_js_1.logger.warning("An error occurred while parsing the message from service", err);
                    throw err;
                }
                ...
                messages.forEach((message) => {
                    try {
                        ...
                        logger_js_1.logger.warning(`An error occurred while handling the message with kind: ${message.kind} from service`, err);
                    }
                });
            });
        });
        ...
        try {
            while (!this._isStopping) {
                try {
                    ...
                    logger_js_1.logger.warning("An attempt to reconnect connection failed.", err);
                    attempt++;
                    const delayInMs = this._reconnectRetryPolicy.nextRetryDelayInMs(attempt);
                    if (delayInMs == null) {
                        break;
                    }
                    logger_js_1.logger.verbose(`Delay time for reconnect attempt ${attempt}: ${delayInMs}`);
                    await (0, core_util_1.delay)(delayInMs).catch(() => {
                        /** empty */
                    });
                }
            }
    ...
    async _sendMessageWithAckId(messageProvider, ackId, abortSignal) {
        ...
        try {
            ...
            throw new index_js_1.SendMessageError(errorMessage, { ackId: ackId });
        }
        if (abortSignal) {
            try {
                return await (0, abortablePromise_js_1.abortablePromise)(entity.promise(), abortSignal);
            }
            catch (err) {
                if (err instanceof Error && err.name === "AbortError") {
                    throw new index_js_1.SendMessageError("Cancelled by abortSignal", { ackId: ackId });
                }
                throw err;
            }
        }
    ...
    async _handleConnectionClose() {
        ...
        this._ackMap.forEach((value, key) => {
            if (this._ackMap.delete(key)) {
                value.reject(new index_js_1.SendMessageError("Connection is disconnected before receive ack from the service", {
                    ackId: value.ackId,
                }));
            }
        });
        if (this._isStopping) {
            logger_js_1.logger.warning("The client is stopping state. Stop recovery.");
            this._handleConnectionCloseAndNoRecovery();
            return;
        }
        if (this._lastCloseEvent && this._lastCloseEvent.code === 1008) {
            logger_js_1.logger.warning("The websocket close with status code 1008. Stop recovery.");
            this._handleConnectionCloseAndNoRecovery();
            return;
        }
        if (!this._protocol.isReliableSubProtocol) {
            logger_js_1.logger.warning("The protocol is not reliable, recovery is not applicable");
            this._handleConnectionCloseAndNoRecovery();
            return;
        }
        // Build recovery uri
        ...
        if (!recoveryUri) {
            logger_js_1.logger.warning("Connection id or reconnection token is not available");
            this._handleConnectionCloseAndNoRecovery();
            return;
        }
        ...
        try {
            while (!abortSignal.aborted || this._isStopping) {
                try {
                    ...
                    await (0, core_util_1.delay)(1000);
                }
            }
        }
        finally {
            if (!recovered) {
                logger_js_1.logger.warning("Recovery attempts failed more then 30 seconds or the client is stopping");
                this._handleConnectionCloseAndNoRecovery();
            }
        }
    }
    ...
    _buildDefaultOptions(clientOptions) {
        ...
        if (clientOptions.protocol == null) {
            clientOptions.protocol = (0, index_js_2.WebPubSubJsonReliableProtocol)();
        }
    ...
    _changeState(newState) {
        logger_js_1.logger.verbose(`The client state transfer from ${this._state.toString()} to ${newState.toString()}`);
...
exports.WebPubSubClient = WebPubSubClient;
class RetryPolicy {
    ...
    async _start() {
        ...
        while (!signal.aborted) {
            try {
                ...
                await (0, core_util_1.delay)(this._interval);
            }
        }
    }
}
